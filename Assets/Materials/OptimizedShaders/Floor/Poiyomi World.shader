Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/9cdc6d219d427f64dab8e8fa6b28379e"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_REALISTIC 
			...
			{
				float MainColorFeatherStep = float(1) - float(1);
				float firstColorFeatherStep = float(0.317) - float(1);
				#if defined(PROP_1ST_SHADEMAP) || !defined(OPTIMIZER_ENABLED)
				...
				firstShadeMap.rgb *= float4(0.4622642,0.4622642,0.4622642,1).rgb; //* lighColor
				secondShadeMap.rgb *= float4(0.1792453,0.1792453,0.1792453,1).rgb; //* LightColor;
				float shadowMask = 1;
				...
				float firstSecondShadowMask = saturate(1 - ((poiLight.lightMap) - firstColorFeatherStep) / (float(0.317) - firstColorFeatherStep) * (shadowMask));
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.6226415,0.6226415,0.6226415,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.6226415,0.6226415,0.6226415,1).a;
