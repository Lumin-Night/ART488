Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/63fdce4102ce2fb4e9915a556a8bdb61"
{
	Properties
	{
		...
		[HideInInspector][ToggleUI]_PBRMapsStochastic ("Stochastic Sampling", Float) = 0
...
#pragma exclude_renderers gles
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, poiMods, float(0.118), float(0), float(0.004), float(0), float(0), matcap, matcapMask, float(0), float(0), float(0), float(2));
				#endif
				//ifex float(1)==0 && float(0)==0 && float(0)==0 && float(0)==0
			}
			#endif
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0.09351751,0,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0.09351751,0,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0.09351751,0,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			{
				float smoothness = float(0.782);
				float smoothness2 = float(1);
				float metallic = float(1);
				float specularMask = 1;
				float reflectionMask = 1;
				#if defined(PROP_MOCHIEMETALLICMAPS) || !defined(OPTIMIZER_ENABLED)
				float4 PBRMaps = POI2D_SAMPLER_PAN_STOCHASTIC_TOGGLE(_MochieMetallicMaps, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0).xy, float(0));
				
				if (float(0))
				{
					PBRMaps.zw = POI2D_SAMPLER_PAN_STOCHASTIC_TOGGLE(_MochieMetallicMaps, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0).xy, float(0)).zw;
				}
				metallic *= PBRMaps.r;
				smoothness = (smoothness * PBRMaps.g);
				smoothness2 = (smoothness2 * PBRMaps.g);
				reflectionMask *= PBRMaps.b;
				specularMask *= PBRMaps.a;
				#endif
				reflectionMask *= float(0.358);
