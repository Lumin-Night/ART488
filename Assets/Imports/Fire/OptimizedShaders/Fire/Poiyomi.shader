Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Toon/7a36e1195f8a3e84196746c98e1ada36"
{
 ...
 #define PROP_EMISSIONMAP1 
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + float(-0.45));
			}
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(0.1,0.1,0,0)), float4(5,-5,0,0)) * float4(poiThemeColor(poiMods, float4(1,0.3296597,0.1367925,1).rgb, float(0)), float4(1,0.3296597,0.1367925,1).a);
				#else
				...
				#endif
				#if defined(PROP_RIM2TEX) || !defined(OPTIMIZER_ENABLED)
				float4 rim2Color = POI2D_SAMPLER_PAN(_Rim2Tex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(5,-4,0,0)) * float4(poiThemeColor(poiMods, float4(0.745283,0.2685716,0.1019491,1).rgb, float(0)), float4(0.745283,0.2685716,0.1019491,1).a);
				#else
