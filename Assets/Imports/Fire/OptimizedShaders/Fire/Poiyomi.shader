Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Toon/7a36e1195f8a3e84196746c98e1ada36"
{
	Properties
	{
		...
		[sRGBWarning]_EmissionMask1 ("Emission Mask--{reference_properties:[_EmissionMask1Pan, _EmissionMask1UV, _EmissionMaskInvert1]}", 2D) = "white" { }
 ...
 #define PROP_EMISSIONMAP1 
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + float(-0.798));
			}
			...
			{
				float MainColorFeatherStep = float(0.72) - float(0.0001);
				float firstColorFeatherStep = float(0.329) - float(0.0001);
				#if defined(PROP_1ST_SHADEMAP) || !defined(OPTIMIZER_ENABLED)
				...
				firstShadeMap.rgb *= float4(0.4528302,0.4528302,0.4528302,1).rgb; //* lighColor
				secondShadeMap.rgb *= float4(0.3018868,0.3018868,0.3018868,1).rgb; //* LightColor;
				float shadowMask = 1;
				...
				float mainShadowMask = saturate(1 - ((poiLight.lightMap) - MainColorFeatherStep) / (float(0.72) - MainColorFeatherStep) * (shadowMask));
				float firstSecondShadowMask = saturate(1 - ((poiLight.lightMap) - firstColorFeatherStep) / (float(0.329) - firstColorFeatherStep) * (shadowMask));
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionStrength1 = float(4);
				float glowInTheDarkMultiplier1 = calculateGlowInTheDark(float(0), float(1), float(1), float(0), float(0), float(0), poiLight);
				#if defined(PROP_EMISSIONMAP1) || !defined(OPTIMIZER_ENABLED)
				if (!float(0))
				{
					emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0,0,1).rgb, float(0));
				}
				else
				{
					emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0,0,1).rgb, float(0));
				}
				#else
				emissionColor1 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0,0,1).rgb, float(0));
				#endif
				if (float(0))
				{
				...
				if (float(0) > 0)
				{
					emissionMask1 = customBlend(emissionMask1, poiMods.globalMask[float(0)-1], float(2));
				}
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				viewDotNormal = pow(viewDotNormal, float(1.99));
				if (float(0) && float(0))
				{
					viewDotNormal += lerp(0, (1 - poiLight.nDotLNormalized) * 3, float(0));
				}
				float rimStrength = float(9.8);
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(0.1,0.1,0,0)), float4(5,-10,0,0)) * float4(poiThemeColor(poiMods, float4(1,0.3296597,0.1367925,1).rgb, float(0)), float4(1,0.3296597,0.1367925,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(1,0.3296597,0.1367925,1).rgb, float(0)), float4(1,0.3296597,0.1367925,1).a);
				#endif
				...
				float rim = 1 - smoothstep(min(float(0.287), rimWidth), rimWidth, viewDotNormal);
				rim *= float4(1,0.3296597,0.1367925,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
				...
				#ifdef POI_RIM2
				viewDotNormal = abs(dot(poiCam.viewDir, lerp(poiMesh.normals[0], poiMesh.normals[1], float(1))));
				
				if (float(0))
				{
					viewDotNormal = 1 - viewDotNormal;
				}
				viewDotNormal = pow(viewDotNormal, float(10));
				if (float(0) && float(0))
				{
					viewDotNormal += lerp(0, (1 - poiLight.nDotLNormalized) * 3, float(0));
				}
				float rim2Strength = float(1.8);
				float rim2Width = lerp( - .05, 1, float(1));
				float rim2BlendStrength = float(1);
				#ifdef POI_AUDIOLINK
				
				if (poiMods.audioLinkAvailable)
				{
					rim2Width = clamp(rim2Width + lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[float(0)]), - .05, 1);
					rim2BlendStrength += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[float(0)]);
					rim2BlendStrength += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[float(0)]);
				}
				#endif
				#if defined(PROP_RIM2MASK) || !defined(OPTIMIZER_ENABLED)
				float rim2Mask = POI2D_SAMPLER_PAN(_Rim2Mask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				float rim2Mask = 1;
				#endif
				#if defined(PROP_RIM2TEX) || !defined(OPTIMIZER_ENABLED)
				float4 rim2Color = POI2D_SAMPLER_PAN(_Rim2Tex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(20,-10,0,0)) * float4(poiThemeColor(poiMods, float4(0.745283,0.2685716,0.1019491,1).rgb, float(0)), float4(0.745283,0.2685716,0.1019491,1).a);
				#else
				float4 rim2Color = float4(poiThemeColor(poiMods, float4(0.745283,0.2685716,0.1019491,1).rgb, float(0)), float4(0.745283,0.2685716,0.1019491,1).a);
				#endif
				
				if (float(0))
				{
					rim2Color.rgb = hueShift(rim2Color.rgb, float(0) + _Time.x * float(0));
				}
				float rim2 = 1 - smoothstep(min(float(0.112), rim2Width), rim2Width, viewDotNormal);
				rim2 *= float4(0.745283,0.2685716,0.1019491,1).a * rim2Color.a * rim2Mask;
				if (float(0))
				{
					switch(float(0))
					{
						...
						float rampedLightMap = poiLight.rampedLightMap;
						if (float(0)) rampedLightMap = 1 - rampedLightMap;
						rim2 = lerp(rim2, rim2 * rampedLightMap, float(1));
						break;
						case 1:
						float nDotLNormalized = poiLight.nDotLNormalized;
						if (float(0)) nDotLNormalized = 1 - nDotLNormalized;
						rim2 = lerp(rim2, rim2 * smoothstep(float4(0,0,0,1).x, float4(0,0,0,1).y, nDotLNormalized), float(1));
						break;
					}
				}
				float3 finalRim2Color = rim2Color.rgb * lerp(1, poiFragData.baseColor, float(1));
				finalRim2Color *= float(1);
				switch(float(2))
				{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(1,0.3296597,0.1367925,1).rgb, float(0)));
				...
				#ifdef POI_RIM2
				#if defined(PROP_SET_RIM2LIGHTMASK) || !defined(OPTIMIZER_ENABLED)
				float4 _Set_Rim2LightMask_var = POI2D_SAMPLER_PAN(_Set_Rim2LightMask, _MainTex, poiUV(poiMesh.uv[_Set_Rim2LightMaskUV], float4(1,1,0,0)), _Set_Rim2LightMaskPan);
				#else
				float4 _Set_Rim2LightMask_var = float4(1.0, 1.0, 1.0, 1.0);
				#endif
				float3 rim2Color = float3(poiThemeColor(poiMods, float4(0.745283,0.2685716,0.1019491,1).rgb, float(0)));
				float3 _Is_LightColor_Rim2Light_var = lerp(rim2Color, (rim2Color * poiLight.directColor), float(1));
				float _Rim2Area_var = (1.0 - dot(lerp(poiMesh.normals[0], poiMesh.normals[1], float(1)), poiCam.viewDir));
				float _Rim2LightPower_var = pow(_Rim2Area_var, exp2(lerp(3, 0, float(0.1))));
				float _Rim2light_InsideMask_var = saturate(lerp((0.0 + ((_Rim2LightPower_var - float(0.0001)) * (1.0 - 0.0)) / (1.0 - float(0.0001))), step(float(0.0001), _Rim2LightPower_var), float(0)));
				float _VertHalfLambert_var2 = 0.5 * dot(poiMesh.normals[0], poiLight.direction) + 0.5;
				float3 _LightDirection_MaskOn2_var = lerp((_Is_LightColor_Rim2Light_var * _Rim2light_InsideMask_var), (_Is_LightColor_Rim2Light_var * saturate((_Rim2light_InsideMask_var - ((1.0 - _VertHalfLambert_var2) + float(0))))), float(0));
				float _ApRim2LightPower_var = pow(_Rim2Area_var, exp2(lerp(3, 0, float(0.1))));
				float3 ApRim2Color = float3(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)));
				float3 _Rim2Light_var = (saturate((_Set_Rim2LightMask_var.g + float(0))) * lerp(_LightDirection_MaskOn2_var, (_LightDirection_MaskOn2_var + (lerp(ApRim2Color, (ApRim2Color * poiLight.directColor), float(1)) * saturate((lerp((0.0 + ((_ApRim2LightPower_var - float(0.0001)) * (1.0 - 0.0)) / (1.0 - float(0.0001))), step(float(0.0001), _ApRim2LightPower_var), float(0)) - (saturate(_VertHalfLambert_var2) + float(0)))))), float(0)));
				
				if (float(0))
				{
					_Rim2Light_var = hueShift(_Rim2Light_var, float(0) + _Time.x * float(0));
				}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float alphaMask = POI2D_SAMPLER_PAN(_ClippingMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,0.9,0,0)), float4(0,0,0,0)).r;
				if (float(0))
				{
 ...
 #define PROP_EMISSIONMAP1 
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(5) != POI_MODE_TRANSPARENT)
				{
